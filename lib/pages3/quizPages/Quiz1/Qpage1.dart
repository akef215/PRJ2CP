...
Future<Map<String, dynamic>> fetchQuizDataNP() async {
  final response = await http.get(
    Uri.parse(path + 'quizzes/quizzes/2/details'),
  );
  if (response.statusCode == 200) {
    ...
    return {
      'quizId': data['quiz_id'],
      'timeLimit':
          data['time_limit'], // Keep this as it is (already an integer)
...
class _QuizPage1State extends State<QuizPage1> {
  ...
  bool isAnswerCorrect(int choiceId) {
    ...
    return questions[currentPage].choices.any(
      (choice) => choice.choiceId == choiceId && choice.points > 0,
    );
  }
  ...
  Widget buildAnswerOption(List<Choice> choices) {
              ...
              Transform.scale(
                ...
                child: Checkbox(
                  ...
                  onChanged: (bool? value) {
                    setState(() {
                      toggleAnswer(choices[index].choiceId);
                      if (value == true) {
