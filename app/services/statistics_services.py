...
async def get_chapter_statistics_by_student(student_id: str, db: AsyncSession):
    ...
    for chapter in chapters:
        q_query = select(Question).where(Question.chapter_id == chapter.id)
        ...
        for question in questions:
            c_query = select(Choice).where(Choice.question_id == question.id)
            c_result = await db.execute(c_query)
            choices = c_result.scalars().all()

            question_max_score = max((choice.score for choice in choices), default=0)
            total_score += question_max_score

            r_query = select(Result).where(
                Result.student_id == student_id,
                Result.question_id == question.id
            )
            r_result = await db.execute(r_query)
            result = r_result.scalars().first()

            if result:
                answered += 1
                selected_choice = next((c for c in choices if c.id == result.choice_id), None)
                if selected_choice:
                    student_score += selected_choice.score

        pourcentage = (student_score / total_score * 100) if total_score > 0 else 0

        data.append({
            "chapter": chapter.titre,
            "answered": f"{answered}/{total_questions}",
            "percentage": round(pourcentage, 2)
        })

    return data

    
async def get_chapter_statistics(db: AsyncSession):
    query = select(Chapter)
    result = await db.execute(query)
    chapters = result.scalars().all()
    ...
    for chapter in chapters:
        ...
        for question in questions:
            ...
            r_query = select(Result).where(Result.question_id == question.id)
            r_result = await db.execute(r_query)
            ...
            for result in results:
                selected_choice = next((c for c in choices if c.id == result.choice_id), None)
                if selected_choice:
                    total_student_score += selected_choice.score
                    total_answers += 1

        pourcentage = (total_student_score / (total_score * total_answers) * 100) if total_score > 0 and total_answers > 0 else 0

        data.append({
            "chapter": chapter.titre,
            "total_questions": total_questions,
            "total_answers": total_answers,
            "average_score_percentage": round(pourcentage, 2)
        })
