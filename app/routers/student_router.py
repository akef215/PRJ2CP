...
@router.get("/me/Profile", response_model=StudentResponse)  # Update this to use StudentResponse
async def get_my_profile(current_student: dict = Depends(get_current_student)):
    # Make sure the current_student data has the necessary attributes like 'sub' and 'exp'
    return StudentResponse(id=current_student["sub"], code=current_student.get("code", ""), name=current_student.get("name", ""))
